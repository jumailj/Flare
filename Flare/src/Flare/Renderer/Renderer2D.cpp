...
namespace Flare
{
	...
	// todo change texIndex to textureIndex everywhere.
    // primitives
	...
	/****************************************************************************/
	/* 	DRAW QUAD WITH TRANSFORM	*/

	// draw quad with transform + color
	...
	// draw quad with transform + texture;
	...
	//draw  quad with subtexture  + rotation;
	...
	{
			DrawRotatedQuad(position, size, rotation, subtexture, tilingFactor);
	}
	...
	{
			// setting up texture cords and texture;
		constexpr glm::vec4 color = { 1.0f, 1.0f, 1.0f, 1.0f };
		const glm::vec2* textureCoords = subtexture->GetTexCoords();
		const Ref<Texture2D> texture = subtexture->GetTexture();
